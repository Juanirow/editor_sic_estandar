...
def increment_PC(increment):
    ...
    pc.append(hex.plus(num1,increment))
...
def p_directiva_start(p):
    ...
    if parse.pasada == 1:
        ...
        if compare_string_number(s):
            insert_error("1","La direccion inicial debe de ser mayor a cero")
            parse.inicial = "000000"
        else:
            parse.inicial = p[3]
...
## las intrucciones pueden tener una etiqueta y con direccionamiento indexado 
#    o directo con excepcion de RSUB
#@param p arreglo mapeado con los simbolos gramaticales de la regla
def p_instruccion_codigo(p):
  ...
  if parse.pasada == 2:
      ...
      p[0]=string
      step2.insert_str(string,pc[p.lineno(2)-1])
      obj_code.append(string)
  pass
...
def p_instruccion_codigo_RSUB(p):
    ''' CI : RSUB'''
    if parse.pasada==2:
        str = step2.operation_RSUB()
        step2.insert_str(str,pc[p.lineno(1)-1])
        obj_code.append(str)
    pass

## regresa la instruccion que se emparejo
#@param p arreglo mapeado con los simbolos gramaticales de la regla
def p_instruccion(p):
  ''' INSTRUCCION : ADD
  | AND
